...
Since we are using Bootstrap, this is all the code we'll have to create:
...
```
> Such `<script>` tags should be placed **at the end** of your `<body>` tag, but **before** your `.js` inclusions.

We then give it an `id` for future reference, and write our HTML inside:
...
```js
var newMessage = $(template);
```
> `newMessage` now contains a jQuery object representing the desired HTML template.
> 
