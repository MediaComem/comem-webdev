...
function parse(model) {
  ...
  // Parse doctoc comment
  parseElements(model, /^<\!--\s*START\s+doctoc\s*[^\n]+-->[\s\S]+<\!--\s*END\s+doctoc\s+[^\n]+-->/m, MarkdownDoctoc);
  ...
  // Parse slide notes comments
  parseElements(model, /\<\!\-\-\s*slide-notes\s*\-\-\>/mg, MarkdownSlideNotes);
}
...
function MarkdownDoctoc(model, options) {
  MarkdownElement.apply(this, arguments);
  this.type = 'doctoc';
}

util.inherits(MarkdownDoctoc, MarkdownElement);

MarkdownDoctoc.prototype.transform = function() {
  this.replaceWith('');
};



function MarkdownSlideNotes(model, options) {
  MarkdownElement.apply(this, arguments);
  this.type = 'notes';
}

util.inherits(MarkdownSlideNotes, MarkdownElement);

MarkdownSlideNotes.prototype.transform = function() {
  this.replaceWith('???');
};
...
function getNextGridElement(model, index) {
  return model.find(function(element) {
    return _.includes([ 'column', 'container', 'notes', 'title' ], element.type) && element.startIndex > index;
  });
}
